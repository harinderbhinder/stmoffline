...
void updateJogMenu() {
  lcd.clear();
  const char* options[] = {"X+", "X-", "Y+", "Y-", "Back"};
  for (byte i = 0; i < 2; i++) {
    lcd.setCursor(0, i);
    lcd.print((selectedOption + i < 5 && selectedOption == i + selectedOption) ? ">" : " ");
    if (selectedOption + i < 5)
...
void handleJogMenu() {
  ...
  if (digitalRead(selectPin) == HIGH) {
    ...
    switch (selectedOption) {
      ...
      case 4: 
        currentMenu = 0;  // Reset currentMenu to 0 (Main menu)
        selectedOption = 0;  // Reset selectedOption to 0 (Unlock)
