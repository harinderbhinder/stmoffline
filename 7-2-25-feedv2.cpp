...
// ADDED: Variable to track the current feed rate percentage
...
// MODIFIED: This function is completely rewritten to show the feed rate percentage.
void updatePauseDisplay(String filename, int progress) {
  ...
  if (showFeedOverrideMessage && (millis() - lastFeedOverrideTime < 1500)) {
    ...
    if (filePaused) {
      ...
      // Show "Running:" and the current feed rate percentage
      lcd.setCursor(0, 0); lcd.print(F("Running:"));
      String feedStr = "F" + String(currentFeedRate) + "%";
      // Right-align the feed string for a clean look
      lcd.setCursor(16 - feedStr.length(), 0);
      lcd.print(feedStr);
    }
  }
...
// MODIFIED: This function now updates the feed rate variable and message.
void checkFeedOverrideButtons() {
  ...
  if (digitalRead(upPin) == LOW) {
    ...
    if (digitalRead(upPin) == LOW) {
      ...
      if (currentFeedRate < 200) { // Set a maximum of 200%
          currentFeedRate += 10;
      }
  ...
  if (digitalRead(downPin) == LOW) {
    ...
    if (digitalRead(downPin) == LOW) {
      ...
      if (currentFeedRate > 10) { // Set a minimum of 10%
          currentFeedRate -= 10;
      }
      feedOverrideMsg = "Feed " + String(currentFeedRate) + "%";
      showFeedOverrideMessage = true;
...
void sendFile(String filename) {
  ...
  fileRunning = true;
  filePaused = false;
  // ADDED: Reset feed rate to 100% at the start of every job
  currentFeedRate = 100;
